function h = plotWaterFall( ax, y, names )
...
set(ax, 'XTick', linspace(1, length(names), length(names)));
%set(ax, 'XTickLabel', names);
t = my_xticklabels(ax, linspace(1, length(names), length(names)), names);
% colors:
...
% last total bar
h(end+1) = bar(ax(1), n+1, cumy(n), 0.6, 'CData', 0, 'LineWidth', 2.5);
...
plot([3.3 3.7], [to(2) to(2)], ':', 'LineWidth', 2.5, 'Color', [0.5 0.5 0.5]);
