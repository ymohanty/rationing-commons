classdef waterCounter
    ...
    properties
        ...
        trim_tails = 2; % Trim 2 percent, 1 percent on each side
    ...
    methods
        ...
        function [ WBetaE ] = productivityObserved( obj )
            ...
            WBetaE = table2array(obj.model.data.clean(:,obj.model.betaYENames)) ...
                        * obj.model.betaYE;
        end
        ...
        function obj = solveFarmersProblemGivenRationCobbDoug( obj, ration )
            ...
            exog_inputs = table2array(obj.fmplot(:,exog_names));
            ...
            endog_prices = table2array(obj.fmplot(:,obj.getPrices(obj.endog)));
            ...
            if strcmp(ration,'constrained')
                ...
                endog_prices = table2array(...
                    obj.fmplot(:,obj.getPrices(endog_names)));
                sum_endog_alphaj = sum(endog_alphaj);
        ...
        function obj = solveFarmersProblemGivenRationTranslog( obj, ration )
            ...
            if isempty( endog_names ) 
                ...
                endog_prices = table2array(...
                    obj.fmplot(:,obj.getPrices(endog_names_exwater)));
                ...
                water_prices = table2array(...
                    obj.fmplot(:,obj.getPrices({'water'})));
                sum_endog_alphaj_exwater = sum(endog_alphaj);
                logCitsAdj = (1-sum_endog_alphaj_exwater)*...
                    log(water_prices) - logCits;
                water_alphaj = [ obj.model.alpha(...
                    ismember(obj.model.input,{'water'}))' ...
                    obj.model.alpha(end) ];

                % Solve for expected output and water demand
                [ wit ] = obj.solveFarmersDemandTranslog( ...
                    logCitsAdj, water_alphaj, sum_endog_alphaj_exwater );

                % Calculate yEit as a function of water use
                yEit = wit + log(water_prices) - ...
                    log(water_alphaj(1)+2*water_alphaj(2)*wit);                   
            end
            obj.fmplot(:,['output' suffix]) = array2table(yEit);
            ...
            if ~isempty( endog_names )
                ...
                if any(ismember(endog_names,{'water'}))
                    ...
                    obj.fmplot(:,endog_wsuffix) = array2table(wit);
                end
            end
        ...
        function obj = solveFarmersProblemReconcile( obj )
            ...
            obj.fmplot(:,Inputs) = ...
                array2table(exp(table2array(obj.fmplot(:,[inputs 'output']))));
        end
        ...
        function obj = calculateProfits( obj, suffix )
            ...
            inputs = table2array(obj.fmplot(:,strcat(input_names,suffix)));
            input_prices = table2array(obj.fmplot(:,obj.getPrices(input_names)));
            obj.fmplot(:,['profit' suffix]) = array2table( ...
                exp(table2array(obj.fmplot(:,['output' suffix]))) - ...
            ...
            if strcmp(obj.policy.regime,'block_pricing')
                ...
                obj.fmplot(:,['profit' suffix]) = ...
                    array2table( table2array(obj.fmplot(:,['profit' suffix])) ...
        ...
        function obj = shadowValueAtMean( obj )
            ...
            exog_inputs = mean(table2array(obj.fmplot(:,obj.exog)),1);
        ...
        function [ dPidH, dPidHi ] = marginalBenefit( obj, alphaW )
            ...
            inputs     = obj.model.inputUnc;            
            inputs_con = table2array(obj.fmplot(:,strcat(inputs,'_con')));
...
%             dPidH_bar = dPidH_bar / mean(obj.model.data.clean.Land);
        ...
        function [ alphaWcalib ] = calibrateAlpha( obj )
            ...
            [ alphaWcalib ] = fminunc( objective, obj.model.alpha(end), ...
                                       options );
        ...
        function obj = aggregateProductivity( obj )
            ...
            waterIdx         = find(ismember(obj.model.input,'water'));
...
function [ Cg, wg ] = solveWaterDemandOngrid( logCitsAdj, ...
                        water_alphaj, sum_endog_alphaj_exwater  )
    ...
    % Define grid points and water output vector
    Npoints = 99;
    Cg = prctile( logCitsAdj, [ 0.5:(Npoints-1)/100:99.5 ]);
    wg = zeros(size(Cg));
    
    lb = 2;
    ub = -water_alphaj(1)/(2*water_alphaj(2));
    w = [0:0.1:9];
    ...
    for g = 1:length(Cg)
        ...
        waterDemand = @( w ) Cg(g) - ...
            log(water_alphaj(1) + 2*water_alphaj(2)*w) + ...
            (1-sum_endog_alphaj_exwater-water_alphaj(1))*w - ...
            water_alphaj(2)*w.^2;

%         fprintf('Iteration %3.0f, constant %6.2f\n',g,Cg(g));
%         plot(w,waterDemand(w)); hold on;
%         plot(w,zeros(length(w)));   
        
        if waterDemand(ub) < 0
            wg(g) = ub; % Water demand at bliss point
        else
            [ wg(g) ] = fzero(waterDemand,[ lb ub ]);
        end
...
function outtab = accumTable( table, groupvar, funcToAccum )
    ...
    data        = table2array(table(:,varsInData));
    columns     = size(data,2);
    ...
    if ~isempty( groupvar )
        ...
        [ ids, ~, pos ] = unique(table2array(table(:,groupvar)),'rows');
