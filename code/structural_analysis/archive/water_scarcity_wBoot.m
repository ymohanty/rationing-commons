...
%% Fit production function model by IV (all inputs endogenous)
%    Bootstrap standard errors at feeder level and calibrate alpha
model = waterModel(water);
...
model.ivSet            = 'logInvDepthPDS';
model.noisy            = false;
model.bootIterations   = 100;

% Estimate model
model = model.estimateIV;
model = model.estimateIVBoot;
...
counter        = waterCounter( model );
counter.policy = waterPolicy('rationing');
model.alpha(end) = calibrateAlpha(counter,2.190);

% Bootstrap SEs and store in file (comment out after one run)
se = bootstrapAlpha(water,100);
fileID = fopen('bootstrap_se.txt','w');
fprintf(fileID,'%f',se);
fclose(fileID);

fileID = fopen('bootstrap_se.txt','r');
model.bootstrapWaterSE = fscanf(fileID,'%f');

modelCalibratedAlpha = model;
...
cd(tables);
estimates = { modelOls, modelIvWat, modelIvAll, modelCalibratedAlpha };
