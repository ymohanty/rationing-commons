/*******************************************************************************
Purpose: Calibrate the toy dynamic model 
...
if c(mode) == "batch" {
	...
	if c(os) == "Windows" {
		...
		local PROJECT_ROOT "C:/Users/`c(username)'/Dropbox/water_scarcity"
	}
	else {
		cd "/Users/`c(username)'/Dropbox"
		local PROJECT_ROOT "/Users/`c(username)'/Dropbox/water_scarcity"
	}
}

include "`PROJECT_ROOT'/analysis/code/load_file_paths.do"

// Exhibits path

local TABLES "`TABLES'/opportunity_cost_water"

// Use the cleaned version of the data
	
use "`CLEAN_FARMER_SURVEY'/farmer_crop_with_imputed_variables.dta", clear
drop SDO_num
...
// Only keeping the relevant variables for analysis
	...
	lpoly depth year_dug if SDO == "Dug"
	...
	keep year_dug depth
	sort year_dug depth
	order year_dug depth
	export delimited "`WORKING_DATA'/depth_data.txt", novarnames replace
	...
	drop SDO
	export delimited using "`WORKING_DATA'/sdo_initial_conditions.txt", novarnames replace
	restore
	
	// Store sample-wide exogenous variables as MATLAB input 
	collapse (mean) depth mean_pump_capacity tot_water_liter rho_tilde 
	gen rho_tilde_bar = tot_water_liter*depth/(42*6*mean_pump_capacity_farmer)
	export delimited using "`WORKING_DATA'/init_conditions.txt", novarnames replace
