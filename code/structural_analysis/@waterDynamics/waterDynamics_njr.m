classdef waterDynamics
    ...
    properties
        ...
        alpha_hat;                  % Water elasticity of output point estimate
        alpha_b;                    % Water elasticity of output bootstrap
    ...
    methods
        ...
        function obj = waterDynamics(model,data_paths,beta,alpha)
            if nargin > 0
                ...
                if nargin > 2 
                    obj.alpha_b = alpha; % If alpha passed, use that one
                else
                    waterIdx    = ismember(model.iv.xnames,'water');
                    obj.alpha_b = obj.ivb(waterIdx,:)';            
                end
        ...
        function obj = assignProductionModel( obj, model )
            ...
            % Elasticity of output with respect to water alpha_w
            waterIdx    = ismember(model.iv.xnames,'water');
            obj.alpha_b = model.iv.coef(waterIdx);
        ...
        function obj = assignGroundwaterConditions( obj )
            ...
            H_unc = (obj.omega.*obj.alpha_b/...
                (obj.policy.power_price*obj.days_use)).^(1/(1-obj.alpha_b)) .* ...
                (obj.rho_tilde./obj.D_0).^(obj.alpha_b/(1-obj.alpha_b)) .* ...
                1./obj.P;
        ...
        function [ S_t, H_t, W_t, D_t ] = projectDepth( obj, ...
            ...
            D_t(:,1) = D0;
            H_unc = bsxfun( @power, (obj.omega.*obj.alpha_b/...
                (obj.policy.power_price*obj.days_use)), 1/(1-obj.alpha_b) ) .* ...
                bsxfun( @power, obj.rho_tilde./D_t(:,1), ...
                obj.alpha_b/(1-obj.alpha_b) ) .* 1./mean(obj.P);
            H_t(:,1) = min(H_unc,obj.policy.ration);
            W_t(:,1) = obj.rho_tilde .* mean(obj.P) .* H_t(:,1)./D_t(:,1);
            S_t(:,1) = obj.beta.^(1-1) .* ( obj.omega .* ...
                bsxfun( @power, W_t(:,1), obj.alpha_b ) ...
            ...
            for t = 2:T
                ...
                H_unc = bsxfun( @power, (obj.omega.*obj.alpha_b/...
                    (obj.policy.power_price*obj.days_use)), 1/(1-obj.alpha_b) ) .* ...
                    bsxfun( @power, obj.rho_tilde./D_t(:,t), ...
                    obj.alpha_b/(1-obj.alpha_b) ) .* 1./mean(obj.P);
%                 H_unc = (obj.omega.*obj.alpha_b/...
%                     (obj.policy.power_price*obj.days_use)) ...
%                     .^(1/(1-obj.alpha_b)) .* ...
%                     (obj.rho_tilde./D_t(:,t)).^(obj.alpha_b/(1-obj.alpha_b)) .* ...
%                     1./mean(obj.P);
                ...
                S_t(:,1) = obj.beta.^(1-1) .* ( obj.omega .* ...
                    bsxfun( @power, W_t(:,t), obj.alpha_b ) ...
                    - obj.policy.power_cost.*obj.P.*H_t(:,t)*obj.days_use);
%                 S_t(:,t) = obj.beta.^(t-1) .* ...
%                     ( obj.omega.*(W_t(:,t)).^(obj.alpha_b) ...
%                       - obj.policy.power_cost.*obj.P.*H_t(:,t)*obj.days_use);
            end
