...
%
%   est = IV2SLS(y, X, Z) computes an IV2SLS estimation of y into X, using the
...
%   - 'constant': 0 to omit the constant term (intercept) in the
%   estimation. Default 1.
%   - 'vartype': computer cluster-robust var-cov matrix with 'cluster'.
%               compute heteroskedasticity 'robust' variance matrix 
...
%   - 'clustervar': variable to cluster if option 'cluster' is used.
%   
%
%   Example
%     
...
%   See also ESTOUT, OLS, S2SLS, IVPANEL
    ...
    if size(y,2) ~= 1
        error('Y must be a column vector of the dependent variable');
    end
    if size(y,1) ~= size(X)
    ...
    addPar(p,'clustervar',[],@(x) assert(size(x,1) == size(y,1) && size(x,2) == 1, 'Cluster variable must be a vector of the same length as Y' ))
    p.parse(varargin{:})
    options = p.Results;
    
    % Ensure cluster-variable is provided if using cluster robust
    % inference.
    try
        if strcmp(options.vartype,'cluster') && isempty(options.clustervar)
            error('Provide cluster variable for cluster-robust inference')
        end
    catch
        error('Provide cluster variable for cluster-robust inference')
    end
    ...
    switch options.vartype
        ...
        case 'cluster'
            % Identify clusters
            [clustervar, ~] = extracttable(options.clustervar);
            g = unique(clustervar);
            G = size(g,1);
            ...
            % Loop over all clusters, throw meat into sandwich
            for i = 1:G
                Xhat_g = Xhat(clustervar == g(i),:);
                res_g = res(clustervar == g(i));
                B_clu = B_clu + (Xhat_g' * (res_g * res_g') * Xhat_g);
            end
            
            % White-clustered variance-covariance matrix
            varcoef = invXhatX * B_clu * invXhatX;      
    end
    ...
    est.varcoef = varcoef;
    est.stderr = stderr;
    est.tStat = coef ./ stderr;
    est.pValue = 2*(1-tcdf(abs(est.tStat),resdf));
    est.Xhat = Xhat;
    est.yhat = yhat;
    est.res = res;
    est.resvar = resvar;
