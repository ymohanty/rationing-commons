...
for (ti = 1; ti <= Ntime; ti++){
	...
	for (i = 1; i <=n1; i++){
		...
		XeeXh_2 = ((X1[i,.]'*J(1,n1,1)*e1[i,1]):*(J(k,1,1)*e1':*window_i_2'))*X1
		XeeXh_3 = ((X1[i,.]'*J(1,n1,1)*e1[i,1]):*(J(k,1,1)*e1':*window_i_3'))*X1

		//add each new k x k matrix onto the existing matrix (will be symmetric)
	
		XeeX = XeeX + XeeXh
		XeeX_2 = XeeX_2 + XeeXh_2
		XeeX_3 = XeeX_3 + XeeXh_3
...
XeeX_spatial = XeeX / n
XeeX_spatial_2 = XeeX_2 / n
XeeX_spatial_3 = XeeX_3 / n

V = invXX * XeeX_spatial * invXX / n
V_2 = invXX * XeeX_spatial_2 * invXX / n
V_3 = invXX * XeeX_spatial_3 * invXX / n
...
st_matrix("V_spatial_2",V_2)
st_matrix("V_spatial_3",V_3)
...
matrix colnames V_spatial_2 = `X'
matrix rownames V_spatial_2 = `X'

matrix colnames V_spatial_3 = `X'
matrix rownames V_spatial_3 = `X'
